function [pdata, cback] = LearningCurve_WinBugs_230829_IO90(Responses,Chance)
...
%% Output data
% pdata : estimate learning curve & confidence interval of upper 90% and lower 10%
...
for t = 1:length(Responses) 
        ...
        ll           = sort_samples(fix(0.1*total));  %lower 90%interval
        ml           = sort_samples(fix(0.5*total));
        ul           = sort_samples(fix(0.90*total));
