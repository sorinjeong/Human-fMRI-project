...
path_out_1st = fullfile(root_path,'240215','1st_Level'); % save betas this folder
path_out_2nd = fullfile(root_path,'240215','2nd_Level'); % save 2nd lev glm results this folder
...
if run_1st_glm == 1
    for i = 1:length(sbj_id_list)
        ...
        matlabbatch{1}.spm.stats.con.consess{2}.tcon.name = 'Hit-Corr_reject';
        matlabbatch{1}.spm.stats.con.consess{2}.tcon.weights = [1 0 -1 0];
        matlabbatch{1}.spm.stats.con.consess{2}.tcon.sessrep = 'none';
        matlabbatch{1}.spm.stats.con.consess{3}.tcon.name = 'Corr_reject-Hit';
...
if run_2nd_glm ==1
    clear matlabbatch;
    % set conditions
    cond_name = {'corr-incorr','Hit-Corr_reject', 'Corr_reject-Hit'};
    
    % make loop for each conditions
    batch_save_file=cell(length(cond_name), 4);
    for ct_con=1:length(cond_name)        
        % set path
        cond_path_out = fullfile(path_out_2nd,cond_name{ct_con}); mkdir(cond_path_out);
        % set files
        ...
        % Name
        matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = cond_name{ct_con};
        % Weights vector
        ...
        matlabbatch{4}.spm.stats.results.conspec.titlestr = cond_name{ct_con};
