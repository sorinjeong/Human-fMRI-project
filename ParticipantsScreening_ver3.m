...
%% all + half accuracy % connecting the lines between subjects
...
% xlabel('Subject')
% ylabel('Accuracy (%)')
% plot(x,y_all,'k-o', x,y_half_L,'b--*','linewidth',1.5, 'MarkerSize',5)
% % Plot the data points that are in fail_group
% plot(x(fail_group),y_all(fail_group),'r*', x(fail_group),y_half_L(fail_group),'r*','MarkerSize',6);
% 
...
% % fail group에 색칠하기
% ax = gca;
% ax.XTick = x;xTick=ax.XTick;
% ax.XTickLabel = cellstr(num2str(x(:)));yLim=ylim;
% 
...
%% 
% %% all + half bias % connect the dots for y_all and y_half %% overall과
% last half만 있는거!!
...
% plot(x,y_all,'ko', 'linewidth',1.5, 'MarkerSize',5, 'MarkerFaceColor','k');
% plot(x,y_half,'ko','linewidth',1, 'MarkerSize',5);
% % Draw a line between each pair of data points
% for i = 1:length(x)
%     if find((i)==fail_group)
...
% ax = gca;
% ax.XTick = x;
% xTick = ax.XTick; ax.XTickLabel = cellstr(num2str(x(:)));yLim = ylim;
% 
% for i = 1:length(fail_group)
%     if fail_group(i) <= length(xTick)
%         text(xTick(fail_group(i)), yLim(1)-0.05*diff(yLim), ax.XTickLabel{fail_group(i)},...
...
legend({'overall','last-half','first-half','','fail group'},'Location','northeast')
