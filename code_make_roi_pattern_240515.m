...
TYPE='HBT'; % 'HBT' or 'FS60' or 'extra'
make_pattern=1; % if 1, pattern will be made
do_analysis=0; % if u want to do pattern analysis, type 1
    ...
    for b=1:numel(beta_names)
        ...
        for r=1:numel(roi_name)
            ...
            pattern_roi=curr_beta(curr_roi_mask==1);
             ...
             ODT_pattern.(TYPE).(beta_names{b}).(roi_name{r}){sbj_i}=pattern_roi;
            roi_names.(TYPE){r,1}=roi_name{r};
        end
    end
end
...
if do_analysis==1
    ...
    load(fullfile(glm_path,'../raw_ODT_pattern_240515'))
    corr=struct;
...
for c = 1:numel(corr_name_list)
    ...
    writecell(C, fullfile('Z:\E-Phys Analysis\fMRI_ocat\OCAT_DIR\data\new_ocat_glm\240512_new\ODT\pattern','patterns.xlsx'), 'Sheet', corr_name_list{c});
end
