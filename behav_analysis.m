%% root 수기지정
addpath('D:\leelab\Human fMRI projects\MATLAB\23.11.09_Behav&DICOM2BIDS');
cd('Z:\E-Phys Analysis\fMRI_ocat\BehavData_analyzed\ver_231109');
load('Allsub_NumLogTable.mat');T=total_NumLogTable;%clear("total_NumLogTable");

% subject 21.1 다시하기 전 -> FAIL group으로 변경!!!
SubInfoFile.PASS(21)=1;
% subject 87 epilepsy 병력 피험자 -> PASS group으로 변경!!!
SubInfoFile.PASS(3)=1;
%%T에 Pass/Fail, combi 정보 넣기
combi= str2num(strcat(num2str(T.Context),num2str(T.Object)));
T=addvars(T,repelem(SubInfoFile.PASS(:),32),combi,repmat([1;2;3;4],8*28,1),repmat([1;2;1;2],8*28,1),'NewVariableNames',{'PASS','Combi','StopPoint','1324'});
...
Combination=[];
for i=1:length(Subs)
    varName = strcat("sub",num2str(Subs(i), '%02.f'));
...
for n=1:length(P); DataGroup.PASS.(strcat('sub', num2str(P(n), '%02.f'))) = SpliT.(strcat('sub', num2str(P(n), '%02.f'))); end
for n=1:length(F); DataGroup.FAIL.(strcat('sub', num2str(F(n), '%02.f'))) = SpliT.(strcat('sub', num2str(F(n), '%02.f'))); end
...
%     varName = sprintf('sub%.15g',Subs(pass_group(i)));
...
[all_accu_p,all_accu_h] = ranksum(allAccuracy(pass_group), allAccuracy(fail_group));
StatResults(1,:) = {"Overall_Accuracy-P/F",all_accu_p,all_accu_h};

% 2. first-half_accuracy - PASS/FAIL group에 대한 Wilcoxon rank sum test
[first_accu_p,first_accu_h] = ranksum(halfAccuracy_F(pass_group), halfAccuracy_F(fail_group));
...
[all_bias_p,all_bias_h] = ranksum(abs(allbias(pass_group)), abs(allbias(fail_group)));
StatResults(9,:) = {"Overall_Bias-P/F",all_bias_p,all_bias_h};

% 10. first-half_bias - PASS/FAIL group에 대한 Wilcoxon rank sum test
[first_bias_p,first_bias_h] = ranksum(abs(halfbias_F(pass_group)), abs(halfbias_F(fail_group)));
StatResults(10,:) = {"First-half_Bias-P/F",first_bias_p,first_bias_h};

% 11. last-half_bias - PASS/FAIL group에 대한 Wilcoxon rank sum test
[last_bias_p,last_bias_h] = ranksum(abs(halfbias_L(pass_group)), abs(halfbias_L(fail_group)));
StatResults(11,:) = {"Last-half_Bias-P/F",last_bias_p,last_bias_h};

% 12. last-half_bias - PASS/FAIL group에 대한 Wilcoxon rank sum test (#90 제외)
[last_bias_p,last_bias_h] = ranksum(abs(halfbias_L(pass_group)), abs(halfbias_L(fail_group([1 2 4],1))));
StatResults(12,:) = {"Last-half_Bias-P/F",last_bias_p,last_bias_h};
...
% for i = 1:length(figs)
%     saveas(figs(i), ['D:\leelab\Human fMRI projects\목요일 미팅자료\231109\figure_notfail\' num2str(i) '.png']);
% end
% % 
%% 모든 figure 핸들 가져오기
...
%     end
% 
%     % 파일 이름 생성 (공백을 밑줄로 대체, 슬래시를 대시로 대체)
%     filename = strrep(strrep([title_main, '_', title_sub], ' ', '_'), '/', '-') + ".png";
% 
% 
%     % figure를 PNG 파일로 저장
%     saveas(figs(i), ['D:\leelab\Human fMRI projects\목요일 미팅자료\231109\title_figure\' filename]);
% end



% writetable(abs(SubInfoFile),'sorin_ver5_SubInfoFile.csv')