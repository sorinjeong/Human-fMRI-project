...
run_2nd_glm = 0; % if you want to run the 1st glm step, put 1
...
%% contrasts!!
contrast{1,1}='name: ';
contrast{1,2}={'correct';'incorrect';'Hit'; 'correct_rejection';'Corr-Incorr';'Incorr-Corr';'Hit-Corr_reject';'Corr_reject-Hit'};% Add or modify this list based on your conditions
contrast{2,1}='weight: ';
contrast{2,2}={[0.5 0 0.5 0]; [0 0.5 0 0.5]; [1 0 0 0]; [0 0 1 0]; [0.5 -0.5 0.5 -0.5]; [-0.5 0.5 -0.5 0.5]; [1 0 -1 0]; [-1 0 1 0]};  % Add or modify this matrix based on your weights for each condition
...
condition_path = fullfile(root_path, date,['data_for_glm_' date]);mkdir(condition_path);addpath(condition_path);

% load regressors and save contrasts
...
if run_1st_glm == 1
    for i = 1:length(sbj_id_list)
        ...
        for c = 1:length(contrast{1,2})
            ...
            if strcmp(contrast{1,2}{c}, 'Corr-Incorr')
                if sbj_id_list(i)==7
                    matlabbatch{1}.spm.stats.con.consess{c}.tcon.weights = [0.5 0 0.5 0];
                end
                if sbj_id_list(i)==25
                    matlabbatch{1}.spm.stats.con.consess{c}.tcon.weights = [0.5 -1 0.5 0];
                end
            end
            % if you want to Create a different SPM.mat file for each condition
            %              matlabbatch{1}.spm.stats.con.delete = 1;
            %
            %         spm_jobman('run', matlabbatch)
            %         load(fullfile(current_beta_out, 'SPM.mat'));
            %         save(char(fullfile(current_beta_out, ['SPM_' contrast{1,2}{c} '.mat'])),"SPM")
