...
for ctr_ocpr=1:2
    if ctr_ocpr == 1
       ...
       len = length(spm.Sess.U(4).ons);

    else
        JE = ocpr_Table;
       correct_phase1 = ones(length(spm.Sess.U(1).ons),1);
       len = length(spm.Sess.U(1).ons);
    end

%% correctness
if len < 40
    Table = readmatrix([datafolder '\' filenameo '\' JE],"Range",'A2:O40');
else
...
for ro = 1:height(Table)
    ...
    elseif (JE_effidx(ro) >= mean(JE_effidx)) && (JE_correct_ph3(ro)==1)
       ...
       if ctr_ocpr == 1
           ...
           spm.Sess.U(6).ons(:,2) = JE_correct_ph3;
           spm.Sess.U(6).ons(:,3) = JE_effidx;
           spm.Sess.U(6).ons(:,4) = Group;
       else
           spm.Sess.U(1).ons(:,2) = ones(length(spm.Sess.U(1).ons),1);
           spm.Sess.U(2).ons(:,2) = JE_correct_ph2;
           
           spm.Sess.U(3).ons(:,2) = JE_correct_ph3;  
           spm.Sess.U(3).ons(:,3) = JE_effidx;
